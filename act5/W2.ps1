...
New-LocalUser -Name $usuario -Password $pass -FullName $usuario | Out-Null
Add-LocalGroupMember -Group "ftpusers" -Member $usuario -ErrorAction SilentlyContinue
...
if(!(Test-Path $home)){
New-Item -Path $home -ItemType Directory | Out-Null
}

# limpiar links viejos
cmd /c "rmdir /s /q $home\General" 2>$null
cmd /c "rmdir /s /q $home\$grupo" 2>$null
